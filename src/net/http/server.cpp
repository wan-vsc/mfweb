...
coro::task<void> server::serve_connection(io::native_socket s, std::string prefix,
    ...
    // **实测零收益**，却会抬高每条静态连接的缓冲区上限，与"海量连接"这个目标冲突，
    // 因此撤掉。大文件吞吐的现状与瓶颈定位见 03_性能实测报告.md §4.6。
    request_parser parser;
    // 每连接读缓冲的**起步**大小。
    //
    // 为什么不能一上来就要 8 KiB：这是**每连接常驻**的开销。
    // 改之前实测：208,892 条连接时服务端 RSS 1,941 MB，即**每连接约 9.4 KB**，
    // 其中最大的一块就是这个缓冲。改成 2 KiB 起步之后，同样口径下降到
    // **每连接约 3.5 KB**（峰值 408,518 条连接时 RSS 1,439 MB）。
    // 这个常数直接决定连接容量的上限。
    //
